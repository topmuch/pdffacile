...
const DROPZONE_HINTS: Partial<Record<string, string>> = {
  word2pdf: "Fichier .docx uniquement",
  pptx2pdf: "Fichier .pptx uniquement",
  excel2pdf: "Fichiers .xlsx ou .csv",
};
...
export function ToolWorkspace({
  ...
  const showInvalidFilesToast = React.useCallback(
    (rejected: string[]) => {
      toast({
        title: "Format de fichier non pris en charge",
        description:
          rejected.length === 1
            ? `« ${rejected[0]} » ne correspond pas au format attendu.`
            : `${rejected.length} fichiers ignorés : format non pris en charge.`,
        variant: "destructive",
      });
    },
    [toast]
  ...
  const canRun = React.useMemo(() => {
    ...
    return true;
  }, [entries.length, tool.minFiles]);

  const run = async () => {
    if (!canRun || !mainFile) return;
    setPhase("processing");
    setProgress(0);
    setProgressMsg("");
    ...
    try {
      ...
      switch (tool.id) {
        ...
        case "word2pdf":
          res = await proc.wordToPdf(mainFile, onProgress);
          break;
        case "pdf2excel":
          res = await proc.pdfToExcel(mainFile, onProgress);
          break;
        case "excel2pdf":
          res = await proc.excelToPdf(mainFile, onProgress);
          break;
        case "pdf2jpg":
          res = await proc.pdfToJpgs(mainFile, options.imageScale, onProgress);
          break;
        case "jpg2pdf":
          ...
          break;
        case "pptx2pdf":
          res = await proc.pptxToPdf(mainFile, onProgress);
          break;
        default:
    ...
    return (
      <div className="space-y-6" style={colorStyle}>
        <FileDropzone
          ...
          onFiles={addFiles}
          onInvalidFiles={showInvalidFilesToast}
          color={tool.color}
          hint={
            tool.multiple
              ? `Jusqu'à ${tool.maxFiles} fichiers — ${
                  tool.id === "jpg2pdf" ? "images JPG" : "PDF"
                } uniquement`
              : DROPZONE_HINTS[tool.id] ?? "Un seul fichier"
          }
        />
        {tool.id === "jpg2pdf" && (
          <p className="text-center text-xs text-muted-foreground">
            Formats acceptés : JPG / JPEG — l&apos;ordre des images pourra être
            ajusté ensuite.
          </p>
        )}
      </div>
